...
// Data is fetched live from Supabase per user — see Dashboard component
...
const EmptyState = ({ icon, title, sub, action, onAction }) => (
  <div style={{ textAlign:"center", padding:"60px 20px" }}>
    ...
    <h3 style={{ fontFamily:"'Lora',serif", fontSize:22, fontWeight:700, color:C.brown, marginBottom:8 }}>{title}</h3>
    <p style={{ color:C.brownLight, fontSize:14, marginBottom:24 }}>{sub}</p>
...
const Dashboard = ({ setPage }) => {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      setLoading(true);
      const [{ data: proj }, { data: msgs }, { data: invs }] = await Promise.all([
  ...
  const amountDue     = invoices.filter(i => i.status !== "Paid").reduce((s, i) => s + i.amount, 0);
  ...
  const LoadSpin = () => (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", padding:"60px 0", gap:12, color:C.brownLight }}>
      <span style={{ width:20, height:20, border:"2px solid "+C.amber, borderTopColor:"transparent", borderRadius:"50%", display:"inline-block", animation:"spin 0.7s linear infinite" }}/>
      Loading…
    </div>
  );
        ...
        {tabs.map(t => (
          <div key={t.id} onClick={() => setTab(t.id)}
            ...
            {t.id==="messages" && unreadCount > 0 && (
              <span style={{ position:"absolute", right:16, background:C.amber, color:"#fff", borderRadius:99, padding:"2px 7px", fontSize:11, fontWeight:700 }}>{unreadCount}</span>
            )}
          </div>
        ))}
      ...
      <div style={{ flex:1, padding:"40px", overflowY:"auto", background:C.cream }}>
        ...
        {tab==="overview" && (
          <div style={{ animation:"fadeUp 0.4s ease" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:20, marginBottom:36 }}>
              {[
                { label:"Active Projects", val: loading ? "…" : activeProjs.length,                                  icon:"📂", color:C.amberPale },
                { label:"Amount Due",      val: loading ? "…" : amountDue > 0 ? "$"+amountDue.toLocaleString() : "$0", icon:"💸", color:"#FEE2E2" },
                { label:"Messages",        val: loading ? "…" : unreadCount+" new",                                  icon:"💬", color:C.greenPale },
                { label:"Total Invoices",  val: loading ? "…" : invoices.length,                                     icon:"🧾", color:C.goldPale },
              ].map(s => (
            ...
            <Card style={{ marginBottom:24 }}>
              ...
              {loading ? <LoadSpin/> : activeProjs.length === 0
                ? <EmptyState icon="📂" title="No active projects yet" sub="Start a project to see it here." action="Browse Services" onAction={() => setPage("services")}/>
                : activeProjs.map(p => (
                  <div key={p.id} style={{ display:"flex", alignItems:"center", gap:16, padding:"16px 0", borderBottom:"1px solid "+C.border }}>
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ fontSize:13, color:C.brownLight, marginTop:2 }}>{p.tier} · Due {p.due_date || "TBD"}</div>
                      <div style={{ marginTop:8, background:C.cream, borderRadius:99, height:6, overflow:"hidden" }}>
                        <div style={{ width:(p.progress||0)+"%", height:"100%", background:C.amber, borderRadius:99 }}/>
                      </div>
                      <div style={{ fontSize:12, color:C.brownLight, marginTop:4 }}>{p.progress||0}% complete</div>
                    </div>
            ...
            <Card>
              ...
              {loading ? <LoadSpin/> : messages.length === 0
                ? <EmptyState icon="💬" title="No messages yet" sub="Messages from your project team will appear here."/>
                : messages.slice(0, 3).map(m => (
                  <div key={m.id} onClick={() => markRead(m.id)}
                    style={{ display:"flex", gap:14, padding:"12px 16px", borderRadius:12, marginBottom:8, cursor:"pointer",
                      background: m.unread ? C.amberPale : C.cream2 }}>
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ display:"flex", justifyContent:"space-between" }}>
                        ...
                        <span style={{ fontSize:12, color:C.brownLight }}>{fmtDate(m.created_at)}</span>
                      </div>
                      <div style={{ fontSize:13, color:C.brownLight, marginTop:4 }}>{m.text}</div>
                    </div>
                    {m.unread && <div style={{ width:8, height:8, borderRadius:"50%", background:C.amber, flexShrink:0, marginTop:6 }}/>}
                  </div>
                ))
              }
            </Card>
        ...
        {tab==="projects" && (
          <div style={{ animation:"fadeUp 0.4s ease" }}>
            ...
            {loading ? <LoadSpin/> : projects.length === 0
              ? <EmptyState icon="📂" title="No projects yet" sub="Once you start a project it will appear here." action="Browse Services" onAction={() => setPage("services")}/>
              : projects.map(p => (
                <Card key={p.id} hover style={{ marginBottom:20 }}>
                  <div style={{ display:"flex", alignItems:"flex-start", gap:16 }}>
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ fontSize:13, color:C.brownLight, margin:"6px 0 16px" }}>{p.tier} Package · Due {p.due_date || "TBD"}</div>
                      <div style={{ background:C.cream, borderRadius:99, height:8, overflow:"hidden" }}>
                        <div style={{ width:(p.progress||0)+"%", height:"100%", background:p.status==="Completed"?C.green:C.amber, borderRadius:99 }}/>
                      </div>
                      <div style={{ fontSize:13, color:C.brownLight, marginTop:6 }}>{p.progress||0}% complete</div>
                    </div>
                  </div>
                </Card>
              ))
        ...
        {tab==="messages" && (
          <div style={{ animation:"fadeUp 0.4s ease" }}>
            ...
            {loading ? <LoadSpin/> : messages.length === 0
              ? <EmptyState icon="💬" title="No messages yet" sub="Your project team will send updates here."/>
              : messages.map(m => (
                <Card key={m.id} style={{ background: m.unread ? C.amberPale : C.white, marginBottom:12 }}>
                  <div style={{ display:"flex", gap:16, cursor:"pointer" }} onClick={() => markRead(m.id)}>
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                        ...
                        <span style={{ fontSize:13, color:C.brownLight }}>{fmtDate(m.created_at)}</span>
                      </div>
                      <p style={{ fontSize:14, color:C.brownMid, lineHeight:1.6 }}>{m.text}</p>
                    </div>
                    {m.unread && <div style={{ width:10, height:10, borderRadius:"50%", background:C.amber, flexShrink:0, marginTop:4 }}/>}
                  </div>
                </Card>
              ))
            }
        ...
        {tab==="invoices" && (
          <div style={{ animation:"fadeUp 0.4s ease" }}>
            ...
            {loading ? <LoadSpin/> : invoices.length === 0
              ? <EmptyState icon="🧾" title="No invoices yet" sub="Invoices will appear here after your first payment."/>
              : invoices.map(inv => (
                <Card key={inv.id} hover style={{ marginBottom:16 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:16 }}>
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ fontSize:13, color:C.brownLight, marginTop:4 }}>Issued {inv.issued_date}</div>
                    </div>
