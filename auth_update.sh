#!/bin/bash
# ═══════════════════════════════════════════════════
# DevCraft Hub — Real Supabase Auth Update
...
echo -e "\n${BOLD}🔐 DevCraft Hub — Wiring Real Supabase Auth${NC}"
echo "══════════════════════════════════════════"
...
# ══════════════════════════════════════════════════
# 4. src/main.jsx — wrap app in AuthProvider
# ══════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════
# 5. src/App.jsx — wire useAuth into Nav + AuthPage
# ══════════════════════════════════════════════════
      ...
      <div style={{width:"100%",maxWidth:440}}>
        ...
        <Card>
          <div style={{display:"flex",flexDirection:"column",gap:18}}>
            ...
            {success&&<div style={{background:COLORS.greenPale,color:COLORS.green,padding:"10px 14px",borderRadius:8,fontSize:13}}>✅ {success}</div>}
    ...
    <div style={{paddingTop:68,display:"flex",minHeight:"100vh"}}>
      <div style={{width:240,background:COLORS.brown,padding:"32px 0",display:"flex",flexDirection:"column",gap:4,position:"sticky",top:68,height:"calc(100vh - 68px)",overflowY:"auto"}}>
        ...
        <div style={{marginTop:"auto",padding:"24px 20px 0",borderTop:"1px solid rgba(255,255,255,0.1)"}}>
          ...
          <div onClick={handleLogout} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",cursor:"pointer",color:"rgba(255,255,255,0.4)",fontSize:14}}>🚪 Sign Out</div>
        </div>
      </div>
      <div style={{flex:1,padding:"40px",overflowY:"auto",background:COLORS.cream}}>
        {activeTab==="overview"&&(<div style={{animation:"fadeUp 0.4s ease"}}>
          <h1 style={{fontFamily:"'Lora', serif",fontSize:34,fontWeight:700,color:COLORS.brown,marginBottom:4}}>Welcome back, {displayName?.split(" ")[0]||"there"} 👋</h1>
...
# ── Push ──
step "Pushing to GitHub → Vercel will auto-deploy"
git add -A
git commit -m "feat: wire real Supabase auth — login, signup, session persistence, auto-redirect"
git push origin main
ok "Pushed ✓"

echo ""
echo -e "${GREEN}${BOLD}═══════════════════════════════════════════════${NC}"
echo -e "${GREEN}${BOLD}  🔐 Real Auth is LIVE!${NC}"
echo -e "${GREEN}${BOLD}═══════════════════════════════════════════════${NC}"
echo ""
echo -e "  ✅ Sign up creates a real Supabase user"
echo -e "  ✅ Sign in persists across page refreshes"
echo -e "  ✅ Nav shows your name + avatar initial"
echo -e "  ✅ Dashboard shows your real email"
echo -e "  ✅ Auto-redirects if already logged in"
echo -e "  ✅ Sign out clears session completely"
echo ""
echo -e "  ⚠️  Note: Supabase sends a confirmation email on signup."
echo -e "  To skip during testing: Supabase Dashboard →"
echo -e "  Authentication → Providers → Email → Disable 'Confirm email'"
echo ""
